...
static NSInteger kVykeTableCellHeight = 40.0f;

@interface PaySayAlertViewController ()

@end

@implementation PaySayAlertViewController

+ (PaySayAlertViewController *)sharedInstance{
    static PaySayAlertViewController *sharedInstance = nil;
...
- (CGSize)findHeightForText:(NSString *)text havingWidth:(CGFloat)widthValue andFont:(UIFont *)font {
    ...
    if (text) {
        ...
        CGFloat frameHeight = (frame.size.height + 1);
        if (frameHeight < 22) {
            frameHeight = 22.0f;
            size = CGSizeMake(frame.size.width, frameHeight);
        }
    }
    return size;
}
...
- (void)displayAlertViewOnView:(UIView *)baseView withModel:(PaySayAlertModel *)alertModel andCallBack:(AlertButtonAction)onButtonClick {
    ...
    switch (self.alertModel.alertType) {
        ...
        case kVKButtonType:
        {
            buttonHeightOffSet = self.alertModel.kAlertBottomButtonContainerOffSet;
        }
        default:
            break;
    }
...
- (void)addAlertViewButtons:(UIView *)containerView{
    ...
    primaryButton.tag = kVKPrimaryButton;
    ...
    if (self.alertModel.buttonsArray.count > 1) {
        ...
        secondaryButton.tag = kVKSecondaryButton;
...
- (void) showAlertTableViewOn:(UIView *)containerView withModel:(PaySayAlertModel *)alertModel onCompletion:(AlertTableCellSelectionAction)onCompletion {
    self.cellSelectionCallBack = onCompletion;
    self.alertModel = alertModel;
    self.view.backgroundColor = self.alertModel.overLayColor;
    CGFloat tableViewHeight = 0.0f;
    if (self.alertModel.alertTitle.length > 0) {
        tableViewHeight = ((self.alertModel.alertTableEntries.count * self.alertModel.kAlertTableCellHeight) + kVykeTableCellHeight);
    }else{
        tableViewHeight = (self.alertModel.alertTableEntries.count * self.alertModel.kAlertTableCellHeight);
    }
    if (tableViewHeight > (self.view.frame.size.height - 100)) {
        tableViewHeight = (self.view.frame.size.height - 100);
    }
    self.alertModel.alertTableView = [[UITableView alloc] initWithFrame:CGRectMake(0,0,[self getAlertWidth],tableViewHeight)];
...
//    selectedBackView.backgroundColor = self.alertModel.alertTableCellSelectedColor;
//    [self drawBottomBorderForView:selectedBackView];
//    cell.selectedBackgroundView = selectedBackView;
    ...
    if (self.alertModel.alertCellAccesoryImage) {
        if (self.alertModel.shouldDisplayPreviousSelectedIndex) {
            BOOL isSelected = NO;
            if (self.alertModel.selectedIndexes.count > index) {
                isSelected = [[self.alertModel.selectedIndexes objectAtIndex:index] boolValue];
            }
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if (self.alertModel.alertTitle.length > 0) {
        return kVykeTableCellHeight;
    }
    return 0;
}

- (nullable UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    CGRect viewFrame = CGRectMake(0, 0, [self getAlertWidth], kVykeTableCellHeight);
    UIView *headerView = [[UIView alloc] initWithFrame:viewFrame];
    UILabel *titleLable = [[UILabel alloc] initWithFrame:viewFrame];
    titleLable.text = self.alertModel.alertTitle;
